...
def is_supported(url: str) -> bool:
    # yt-dlp يدعم أكثر من 1000 موقع — أي رابط http/https مقبول
...
def _run_ydl(url: str, opts: dict):
    with yt_dlp.YoutubeDL(opts) as ydl:
        ...
        if not os.path.exists(filename):
            # ابحث عن أي ملف تم تحميله حديثاً
...
def _classify_error(err: str, platform: str) -> str:
    if "File is larger than max-filesize" in err:
        return "__SIZE__"
    if "login" in err.lower() or "sign in" in err.lower() or "log in" in err.lower():
        if platform == "instagram":
            return "هذا الفيديو خاص أو يتطلب تسجيل دخول إنستاغرام ❌"
        return "المنصة تطلب تسجيل دخول، جرب رابطاً آخر ❌"
    if "private" in err.lower():
        return "الفيديو/الحساب خاص ❌"
    if "unavailable" in err.lower() or "not found" in err.lower():
        return "الفيديو غير متاح أو محذوف ❌"
    if "geo" in err.lower() or "not available in your country" in err.lower():
...
async def download_video(url: str) -> dict:
    ...
    # نظّف رابط إنستاغرام من معاملات التتبع الزائدة
    if platform == "instagram":
        url = url.split("?")[0] + "/"

    # قائمة المحاولات حسب المنصة
    if platform == "youtube":
        attempts = [
        ...
        ig_base = {
            ...
            "extractor_args": {"instagram": {"include_dash_manifest": ["0"]}},
        }
        attempts = [
            _build_ydl_opts({**ig_base, "format": "best[ext=mp4]/best"}),
            _build_ydl_opts({**ig_base,
                             "http_headers": {**ig_base["http_headers"], "User-Agent": _DESKTOP_UA},
                             "format": "best[ext=mp4]/best"}),
            _build_ydl_opts({**ig_base, "format": "best"}),
        ]
        ...
        attempts = [
            _build_ydl_opts(),
            _build_ydl_opts({"format": "best[height<=480]/best"}),
            _build_ydl_opts({
                "http_headers": {"User-Agent": _MOBILE_UA},
                "format": "best[ext=mp4]/best",
            }),
    ...
    for opts in attempts:
        try:
            ...
            classified = _classify_error(str(e), platform)
            if classified == "__SIZE__":
                return {"success": False, "error": "حجم الفيديو يتجاوز 50MB المسموح به ❌"}
            if classified:
                # أخطاء نهائية — لا فائدة من إعادة المحاولة
                return {"success": False, "error": classified}
            # خطأ عام — تابع للمحاولة التالية
            last_error = "فشل التحميل، تأكد من الرابط ❌"
