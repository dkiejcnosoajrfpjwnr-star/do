...
async def _cobalt_get_link(url: str) -> dict:
    """
    يسأل cobalt.tools عن رابط تحميل مباشر.
    يُعيد: {"ok": True, "dl_url": "...", "filename": "..."} أو {"ok": False, "error": "..."}
    """
    payload = {
        "url": url,
        "videoQuality": "720",
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(
                COBALT_API, json=payload, headers=COBALT_HEADERS, timeout=aiohttp.ClientTimeout(total=20)
            ) as resp:
                if resp.status != 200:
                    return {"ok": False, "error": f"cobalt HTTP {resp.status}"}
        ...
        if status in ("redirect", "tunnel", "stream"):
            ...
            if dl_url:
                return {"ok": True, "dl_url": dl_url, "filename": filename}

        if status == "picker":
            # عدة مقاطع (مثل كاروسيل إنستاغرام) — خذ الأول
        ...
        if status == "error":
            code = data.get("error", {}).get("code", "error.unknown")
            return {"ok": False, "error": code}

        return {"ok": False, "error": "cobalt: استجابة غير متوقعة"}

    except asyncio.TimeoutError:
        return {"ok": False, "error": "cobalt: انتهت المهلة"}
    except Exception as e:
        return {"ok": False, "error": str(e)[:100]}

async def _cobalt_download(url: str) -> dict:
    """
    يحصل على رابط cobalt ثم يحمّل الملف محلياً.
    يُعيد: {"success": True, "path": "...", "title": "..."} أو {"success": False, "error": "..."}
    """
    link_result = await _cobalt_get_link(url)
    if not link_result["ok"]:
        return {"success": False, "error": link_result["error"]}

    dl_url  = link_result["dl_url"]
    raw_name = link_result.get("filename", "video.mp4")
    # اجعل اسم الملف آمناً وفريداً
    safe_name = re.sub(r'[^\w.\-]', '_', raw_name)[:80]
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.get(
                dl_url,
                headers={"User-Agent": COBALT_HEADERS["User-Agent"]},
        ...
        return {"success": True, "path": out_path, "title": title}
...
def _run_ydl(url: str, opts: dict):
    with yt_dlp.YoutubeDL(opts) as ydl:
        ...
        if not os.path.exists(filename):
            files = sorted(Path(TEMP_DIR).glob("*.mp4"), key=os.path.getmtime, reverse=True)
            if files:
...
async def _ytdlp_download(url: str) -> dict:
    ...
    for opts in _ydl_attempts(url):
        try:
            ...
            friendly = _friendly_ydl_error(str(e))
...
#  1) cobalt.tools  2) yt-dlp احتياطياً
# ──────────────────────────────────────────
async def download_video(url: str) -> dict:
    url = _clean_url(url)

    # المحاولة الأولى: cobalt.tools
    logger.info(f"[cobalt] جرب: {url}")
    cobalt_result = await _cobalt_download(url)
    if cobalt_result["success"]:
        logger.info("[cobalt] نجح ✅")
        return cobalt_result
    logger.warning(f"[cobalt] فشل: {cobalt_result['error']} — ننتقل لـ yt-dlp")
